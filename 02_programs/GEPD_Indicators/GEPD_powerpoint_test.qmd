...
# Breakdown for Infrastructure by Urban/Rural
...
teacher_discuss_df <- teachers_dta %>%
  ...
  select(one_of(discuss_indicators)) 
    ...
    sch_ipw<-teachers_dta$teacher_quest_comb_weight 
...
#list countries
countries <- c("PER", "JOR", "RWA", "MDG", "ETH", "SLE", "NER", "PAK-ICT", "PAK-KP", "PAK-Sindh", "PAK-Balochistan")

indicators_dir <- here() %>% dirname()


#read in indicator metadata
indicators <- read_csv(paste(indicators_dir,'Indicators/indicators.csv', sep = "/"))

#read in data for current country
country_data_raw <- read_csv(here("04_GEPD_Indicators",paste0(country,"_GEPD_Indicators_Stata.csv"))) %>%
  transmute(
    Series=Series,
...
country_data <- color_fn('country_data_raw') %>%
  mutate(
...
PAK_KP_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'PAK_KP', ".csv")) %>%
  mutate(country="Pakistan-KP",
         date='2022') 

PAK_ICT_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'PAK_ICT', ".csv")) %>%
  mutate(country="Pakistan-ICT",
         date='2022') 

		 
PAK_Sindh_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'PAK_ICT', ".csv")) %>%
  mutate(country="Pakistan_Sindh",
         date='2022') 
		 

PAK_Balcohistan_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'PAK_ICT', ".csv")) %>%
  mutate(country="Pakistan_Balochistan",
         date='2022') 
		 
		 
NER_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'NER', ".csv")) %>%
  mutate(country="Niger",
         date='2022') 

SLE_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'SLE', ".csv")) %>%
  mutate(country="Sierra Leone",
         date='2022') 

PER_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'PER', ".csv")) %>%
  mutate(country="Peru",
         date='2019')

RWA_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'RWA', ".csv")) %>%
  mutate(country="Rwanda",
         date='2020') 


JOR_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'JOR', ".csv")) %>%
  mutate(country="Jordan",
         date='2019') 


ETH_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'ETH_pooled', ".csv")) %>%
...
MDG_data <- read_csv(paste0(indicators_dir, "/Indicators/GEPD_Indicators_API_", 'MDG', ".csv")) %>%
...
combined_df_raw <- bind_rows(country_data,PAK_KP_data,PAK_ICT_data, NER_data,SLE_data,PER_data,JOR_data, ETH_data, RWA_data, MDG_data)
...
country_col <- tab_color_df %>% select(country_name) %>% pull()
PAK_KP_col <- tab_color_df$`Pakistan-KP`
PAK_ICT_col <- tab_color_df$`Pakistan-ICT`
PAK_Sindh_col <- tab_color_df$`Pakistan-Sindh`
PAK_Balochistan_col <- tab_color_df$`Pakistan-Balochistan`
...
#build table
tab_df <- tab_df  %>%
  select(`Indicator Name`,!!country_name, `Pakistan-KP`,`Pakistan-ICT`,`Niger`,`Sierra Leone`, Peru, Jordan, Rwanda, Ethiopia, Madagascar)
...
ovr_table <- ovr_table %>%
  ...
  bg(j = c('Pakistan-ICT'),
     bg = PAK_ICT_col) %>%
  bg(j = c('Pakistan-Sindh'),
     bg = PAK_Sindh_col) %>%
  bg(j = c('Pakistan-Balochistan'),
     bg = PAK_Balochistan_col) %>%
  bg(j = c('Niger'),
     bg = NER_col) %>%
  bg(j = c('Sierra Leone'),
     bg = SLE_col) %>%
