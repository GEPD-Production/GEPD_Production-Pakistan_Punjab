...
school_dta <- read_dta(here(processed_dir, "School", "Confidential", "Cleaned", paste0("school_", software, ".dta"))) %>%
  #drop non-saber schools
  filter(grepl("SABER school", strata))
...
# save as csv
write_excel_csv(indicator_data, here("04_GEPD_Indicators", paste0(country, "_GEPD_Indicators_", software, "_SABER_Schools.csv")))
...
urban_rural_data <- indicator_data %>%
  filter(Series %in% c(
    paste0(main_indicators, ".1.R"), paste0(main_indicators, ".1.U"),
...
write_xlsx(list_of_tabs, here("04_GEPD_Indicators", paste0(country, "_GEPD_Indicators_", software, "_SABER_Schools.xlsx")))
