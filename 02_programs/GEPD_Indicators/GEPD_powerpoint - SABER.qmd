---
title: "Global Education Policy Dashboard - Pakistan - Punjab 2023 - SABER Schools"
format: 
  pptx:
    fig_height: 8
    fig_width: 10
...
# Teach Score Breakdown

- The following plots the distribution of teacher pedagogical skill based on the Teach scale.

```{r}
#| label: fig_teachdist
#| fig_cap: Distribution of Teacher Pedagogical Skill

#?  The SABER report used 4 categories (“Low is defined as less than 2.5, medium low 2.5-3.5, medium high 3.5-4.5, and high 4.5-5”) and reported the share in each category, both for the overall score and for each of the component scores (culture, instructional, and socioemotional).  They reported in 2018 that 79% of teachers scored 3.5 or above on classroom culture, but only 5% scored that high on instructional and 1% on socioemotional.  It would be useful to know if that has changed.

#get estimates of share of teachers with teach cores less than 2.5, 2.5-3.5, 3.5-4.5, and 4.5-5
teach_dist <- teachers_dta %>%
  mutate(teach_dist=case_when(
    teach_score<2.5 ~ "Low (<2.5)",
    teach_score>=2.5 & teach_score<3.5 ~ "Medium low (2.5-3.5)",
    teach_score>=3.5 & teach_score<4.5 ~ "Medium high (3.5-4.5)",
    teach_score>=4.5 ~ "High (4.5-5)"
  ))  %>%
  as_survey_design(
...
path <- gsub("GEPD_Production-Pakistan_Punjab", "", here() )
...
for (country in countries) {
  assign(paste0(country, "_GEPD_Indicators"), 
         (read_csv(paste0(path, "GEPD_Production-Pakistan_", country, "/04_GEPD_Indicators/PAK_GEPD_Indicators_Stata.csv")  ) %>%
            transmute(
              Series=Series,
...
for (country in countries) {
  assign(paste0(country, "_GEPD_Indicators"), 
         color_fn(paste0(country, "_GEPD_Indicators")) %>%
            mutate(
              country=country,
              date=2023)
